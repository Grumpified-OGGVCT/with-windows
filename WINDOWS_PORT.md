...
| 3 (PM) | 2026-05-28 | **CCodegen.w patched** — headers now wrapped in `#ifdef _WIN32` / `#else`. **Link.w patched** — `link_stage_make_windows_llvm_link_command()` added. **New files created:** `rt/compat_runtime_windows.w` (Win32 CreateProcess-based process runtime, 528 lines), `rt/windows_x86_64.w` (849-line Win32 API runtime backend). All parse/validate successfully in WSL2. **Remaining blocker:** Need `lld` in WSL2 to rebuild the patched compiler from source. Emit-C already working. Next step: Install `lld` in WSL2 and build the patched compiler, then emit a Windows-specific C bundle and compile on Windows with MSVC. |
| 4 (PM) | 2026-05-28 | **WSL2 static LLVM build restarted** — Ubuntu WSL2, cmake+ninja, `clang;lld` project, `AArch64;X86` targets, Release, static (BUILD_SHARED_LIBS=OFF). Log at `/tmp/llvm-build.log`. Currently at [987/4484] (~22%). ETA: 2-3 hours. **Windows bootstrap shim `windows_platform.c` written and compiled** with VS 2019 Build Tools x64 (`cl /W3 /O2 /c`). 541 lines, all 31 `rt_*` functions + helper macros. **Fixed:** `0o200` → `0200` (MSVC doesn't support Python-style octal); `\\` → `'\\\\'` in PATH_JOIN macro. **Scripts written:** `compile-bootstrap.cmd` (MSVC+static LLVM SDK compile pipeline), `tools/emit-c-windows.sh` (WSL2 emit-C bundle for Windows), `.github/workflows/build-windows.yml` (CI: bootstrap-shim, build-llvm-windows, bootstrap-full). **Committed:** d2472fe1. **Next:** Wait for WSL2 static LLVM build to complete (libclang.a + clang binary), then rebuild patched compiler and emit C for Windows target. |
